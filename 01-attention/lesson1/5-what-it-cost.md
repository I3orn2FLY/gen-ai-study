...
*~6 min. Lesson 1, part 5 of 10.*

## The problem

Part 4's model runs and trains, every position at once. Looks finished.

But part 3 deleted the recurrence *knowing* it was doing three jobs — plus a fourth it did so
quietly nobody listed it — and nobody has gone back to check the wreckage against the model that
actually got built. This part is that audit. Every cost
...
The transformer paper (Vaswani et al., 2017 — part 4 built its stripped-down variant) settles
this in its Table 1, for one *layer* — part 3's sense: the attention op alone against one
recurrent layer, not part 4's whole block. It writes $n$ where we write $T$, and gives both
architectures one shared width $d$:

| | Total work | Sequential steps | Max path length |
|---|---|---|---|
...
was safely inside that: $T = 7$ against $d = 64$. Take a modest modern pair instead — $d = 512$,
$T = 1024$ — and attention does about **twice** the arithmetic of the RNN it replaced.
...
> **Total work** — how much arithmetic gets done: the first column. Attention does *more* past
> $T \approx d$.
> **Critical path** — how much of it has to wait in a queue: the sequential-steps column, $1$
> against $T$. This is part 3's throughput argument, and it is the whole victory.
> The last column is part 3's *path length* — any position to any other in one hop. Real, and
...
Part 3's list, checked off against the built model, in the order the proofs come:

| What recurrence gave you | Survives? | |
|---|---|---|
| Length doesn't change the parameter count | **yes** | first — read part 4's shapes |
| Any length, same memory | **no** | second — counted |
| It knew word order | **no** | third — proven on part 4's own ops |
...
weights, with a single exception coming in a moment. If this job had broken too, short-sentence training would never transfer and the whole
design would be dead on arrival. That it holds is what made losing the other two survivable.

The exception, and it's instructive: part 4's **position table** stores one learned vector per
...
order isn't. Position signals *computed* from the index instead of stored per position narrow
that gap — how well is part of what lesson 5 weighs.
...
is why the **context window** — the longest input a model can take at once — is a hard limit
every model ships with. Note the two different ceilings now on the table: the position table's
was a *shape* limit (position $T{+}1$ has no vector), this one is a *cost* limit; a shipped
model's context window is whichever binds first.

*(Scores are still throwaway, as part 1 said — never parameters. But they occupy memory until
the backward pass has used them, and temporary isn't free at this size. Paying this bill down is
section 03's subject.)*
...
never *which word sits where*. And Bahdanau never faced any of this: his $h_j$ came out of RNN
passes over the sentence (one each way), and a recurrence can't help but know order. That's the
thing that got deleted.
...
The quiet one promised at the top. The decoder's recurrence was doing a fourth thing:
...
you now know the exact shape of. No IOUs on the costs — what remains is a question, not a cost.
Part 4 conjured three matrices, $W_Q, W_K, W_V$, and never justified them. Part 1's $h_j$ played
every role at once and worked fine. Why three?
